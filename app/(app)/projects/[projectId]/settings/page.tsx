...
import { useProject, useUpdateProject, useDeleteProject, useAddMember, useRemoveMember } from '@/features/projects/useProjects'
import { Loader2, Trash2, AlertTriangle, UserPlus, UserMinus, Crown } from 'lucide-react'
...
export default function ProjectSettingsPage({ params }: Props) {
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setInviteSuccess(`${inviteEmail.trim()} agregado correctamente.`)
      setInviteEmail('')
    } catch (err: unknown) {
      setInviteError(err instanceof Error ? err.message : 'Error al agregar miembro')
    }
  }
    ...
    <div className="flex-1 overflow-y-auto">
      <div className="mx-auto max-w-lg space-y-8 p-6">
        ...
        <section className="space-y-4">
          <h2 className="text-base font-semibold text-foreground">Miembros del proyecto</h2>

          <div className="space-y-2">
            {(project.project_members ?? []).map((member) => (
              <div key={member.id} className="flex items-center justify-between gap-3 rounded-lg border border-border bg-card px-4 py-2.5">
                <div className="flex items-center gap-2.5 min-w-0">
                  <div className="h-7 w-7 flex-none rounded-full bg-primary/20 flex items-center justify-center text-xs font-semibold text-primary">
                ...
                <div className="flex items-center gap-2 flex-none">
                  {member.role === 'owner' ? (
                    ...
                    <>
                      <span className="rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">Editor</span>
                      <button
          ...
          <form onSubmit={handleInvite} className="space-y-2">
            <label className="text-sm font-medium text-foreground">Agregar colaborador</label>
            <p className="text-xs text-muted-foreground">
              El usuario debe tener cuenta registrada en FasecTracker.
            </p>
            {inviteError && (
              <p className="rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-xs text-destructive">{inviteError}</p>
            )}
            {inviteSuccess && (
              <p className="rounded-md border border-green-500/40 bg-green-500/10 px-3 py-2 text-xs text-green-400">{inviteSuccess}</p>
            )}
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex items-center gap-1.5 rounded-md bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90 transition-colors disabled:opacity-50"
              >
                <UserPlus className="h-4 w-4" />
                {addMember.isPending ? '...' : 'Agregar'}
              </button>
            </div>
          </form>
        </section>
